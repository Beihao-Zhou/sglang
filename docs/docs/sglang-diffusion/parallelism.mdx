...
```text
num_gpus = cfg_parallel_degree × tp_size × ep_size × sp_degree
sp_degree = ulysses_degree × ring_degree
```

Expert parallelism is a first-class, orthogonal axis: it shards MoE experts
across its own `ep_size` ranks and composes with tensor and sequence parallelism
(for example `--ep-size 4 --ulysses-degree 2`).
...
| Expert parallel (EP) | MoE experts, on its own orthogonal axis | all-reduce per MoE block | `--ep-size` |
