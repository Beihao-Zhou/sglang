...
Expert parallelism is the exception to that product: it shards MoE experts across
the ranks a TP group already uses, so it consumes no extra GPUs.
...
| Expert parallel (EP) | MoE experts, over the TP ranks | all-reduce per MoE block | `--ep-size` |
